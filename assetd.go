...
func (t *rateLimitTransport) RoundTrip(req *http.Request) (*http.Response, error) {
	if strings.Contains(req.URL.Host, "roproxy.com") || strings.Contains(req.URL.Host, "roblox.com") {
		t.mu.Lock()
		elapsed := time.Since(t.lastReq)
		if elapsed < 2000*time.Millisecond {
			time.Sleep(2000*time.Millisecond - elapsed)
		}
		t.lastReq = time.Now()
		t.mu.Unlock()
	}
...
func fetchCreatorGames(creatorID int, creatorType string) []int {
	...
	for attempt := 0; attempt < 10; attempt++ {
		...
		if resp.StatusCode == 429 {
			resp.Body.Close()
			time.Sleep(time.Duration(3000*(attempt+1)) * time.Millisecond)
			continue
		}
	...
	if hasAudio {
		...
		for _, af := range audioFormats {
			...
			row.Components = append(row.Components, discordgo.Button{
				Label:    af.Label,
				Style:    style,
				CustomID: "fmt_audio_" + af.Fmt + "_" + uuidStr,
			})
		}
	...
	if hasVideo {
		...
		for _, vf := range videoFormats {
			...
			row.Components = append(row.Components, discordgo.Button{
				Label:    vf.Label,
				Style:    style,
				CustomID: "fmt_video_" + vf.Fmt + "_" + uuidStr,
			})
		}
		rows = append(rows, row)
	}
